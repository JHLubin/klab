...
def set_design(pdb, residue_selectors, args):
	...
	while not jd.job_complete:
		...
		relax_ddg = score_ddg(relaxed_struc, tf)
		des_ddg = score_ddg(pp, tf)
		ddg_change = des_ddg - relax_ddg
		pro_mut = ident_mutations(pose, pp, mc.mutable_residues, 
									relax_res_E_set, des_res_E_set)

		# Adding additional designed score terms
		extra_des_scores = ['residue_scores: ' + str(des_res_E_sum)]
		extra_des_scores += ['ddG: ' + str(des_ddg)]

		# Collecting relaxed score terms
		relaxed_scores = ['relaxed_total_score: ' + str(sf(relaxed_struc))]
		relaxed_scores += read_pose_scores(relaxed_struc)
		relaxed_scores += ['relaxed_residue_scores: ' + str(relax_res_E_sum)]
		relaxed_scores += ['relaxed_ddG: ' + str(relax_ddg)]

		# Collecting differences
		differences = ['score_change: ' + str(score_change)]
		differences += ['residue_score_change: ' + str(res_score_change)]
		differences += ['ddG_change: ' + str(ddg_change)]
