...
odir=/scratch/jhl133/uck2_collaboration/designing_enzyme_round_2; mkdir -p $odir/logs; for ba in c u; do for va in 5am 5cp 5dz 5e c35am cyclo m5 wt; do jobname=a$va$ba; com="python uck2_collaboration/uck_enzyme_design.py -od $odir -l $va$ba -n 5 -x all_ala_\${job} -ala"; python slurmit_BAY.py --job "${jobname}" --partition main --tasks 1 --usearray True --array 1-10 --requeue True --time 1-00:00:00 --begin now --outfiles $odir/logs/${job}_job%a --command "$com"; done; done; for ba in c u; do for va in 5am 5cp 5dz 5e c35am cyclo m5 wt; do jobname=w$va$ba; com="python uck2_collaboration/uck_enzyme_design.py -od $odir -l $va$ba -n 5 -x wt_start_\${job}"; python slurmit_BAY.py --job "${jobname}" --partition main --tasks 1 --usearray True --array 1-10 --requeue True --time 1-00:00:00 --begin now --outfiles $odir/logs/${job}_job%a --command "$com"; done; done
